...
twodaysaftertomorrow=currentYear + "-" + (currentMonth + 1) + "-" + (currentDayOfMonth + 4)+ "T" + time1;
    cy.request({
      ...
      `'{
          ...
              "msgdata": [
                  {
                      "trans-dttm": "2023-05-03T02:30:00",
                      "unitinfo": {
                          "terminal-cd": "PCT",
                          "unit-cat": "CONTAINER",
       ...
       const data = [ 
        ...
        [response.body.Keys[0].Key, "I", "LALB","PCT", "Test12345","","",pickdate1]
       ]
       cy.task("writeToCSV", { 
        name : 'Demo_test',
        rows: data
...
describe('Add Payment Guarante thru Date after clearing', () => {
  it('Re-Add Payment GTD', () => {
    var start_date="";
    var end_date= "";
    ...
    //uploading the file through fixture
     const fixturepath = 'Demo_test.csv'
   ...
   //visit my container watchlist screen
    ...
    //click on the container in the most recent of the list
     cy.get('[style="padding-bottom: 0px;"] > .layout-row.flex > :nth-child(1) > .layout-xs-column > :nth-child(1) > span.layout-row > .pro-lbl-text-16').click({multiple: true})
     cy.wait(15000)
    
     cy.get('[ng-disabled="showLoader"]').click()
     cy.wait(5000)
     cy.get('.flex-45 > .pro-lbl-text-16').contains('N/A')
     cy.get('[ng-click="closeDialog()"] > .material-icons').click()  
     cy.wait(2000)
    //click on edit servies screen
...
//click on submit button
     ...
     cy.wait(5000)
     cy.get('.flex-45 > .pro-lbl-text-16').contains('2023')
     cy.wait(5000)
     close
